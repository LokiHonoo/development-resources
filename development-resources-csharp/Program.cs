...
namespace Honoo
{
    internal class Program
    {
        ...
        {
            int[] a = new int[] { 11, 22, 33, 44, 55, 66, 77, 88, 99 };
            Honoo.Collections.Generic.Combination<int> combination = new Honoo.Collections.Generic.Combination<int>(a, 5);
            Console.WriteLine($"combination m={5} count = {combination.GetCount()}");
            Console.ReadKey(true);
            int[] count = new int[1];
            combination.Output(Created, count);
            Console.WriteLine($"combination output count = {count[0]}");
            Honoo.Collections.Generic.Permutation<int> permutation = new Honoo.Collections.Generic.Permutation<int>(a, 5);
            Console.WriteLine($"permutation m={5} count = {permutation.GetCount()}");
            Console.ReadKey(true);
            count[0] = 0;
